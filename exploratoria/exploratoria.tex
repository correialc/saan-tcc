...
\begin{alineas}
	\item Os segmentos do tipo Anexo podem ser removidos do conjunto de dados;
	\item Os segmentos com valores ausentes são pouco representativos (0,06\% do total) e podem ser removidos do conjunto de dados;
	\item Será necessário tratar caracteres inválidos na etapa de limpeza;
	\item Os tipos de segmento (classes do modelo) variam de um tipo de ato para outro e isso precisa ser considerado nas etapas seguintes;
	\item Existem padrões nos textos que permitem a avaliação, para alguns tipos de segmento, de heurísticas alternativas à utilização de algoritmos de classificação.
\end{alineas}
